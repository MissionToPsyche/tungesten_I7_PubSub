...
const getDocumentFromS3 = async (req, res) => {
  try {
    ...
    const s3Object = await s3
      .getObject({ Bucket: bucketName, Key: document.metadata.s3Key })
      .promise();
...
const uploadDocument = async (req, res) => {
  try {
    ...
    if (existingDocument) {
      return res
        .status(400)
        .json({ error: "A Document already exists with the same title." });
    }
    const newDocument = new Document({
      title,
      abstract,
...
const getDocumentsByUsername = async (req, res) => {
  try {
    ...
    if (!username) {
      return res
        .status(400)
        .json({ error: "Please provide the document owner's username." });
    }
    const documents = await Document.find({
      "metadata.ownerUsername": username,
    });
    if (documents.length === 0) {
      return res
        .status(404)
        .json({ error: "No documents found for the specified owner." });
    }
    res.json(documents);
  } catch (error) {
    res.status(500).json({ error: error.message });
